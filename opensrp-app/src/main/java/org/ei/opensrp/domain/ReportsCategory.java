...
public enum ReportsCategory {
    ...
    PREGNANCY_OUTCOMES("Pregnancy Outcomes", asList("LIVE_BIRTH", "STILL_BIRTH", "EARLY_ABORTIONS", "LATE_ABORTIONS",
            "SPONTANEOUS_ABORTION", "DELIVERY", "INSTITUTIONAL_DELIVERY", "D_HOM", "D_SC", "D_PHC", "D_CHC", "D_SDH",
            "D_DH", "D_PRI", "CESAREAN", "CESAREAN_GOV", "CESAREAN_PRI")),
    ...
    ReportsCategory(String description, List<String> indics) {
        this.description = description;
        indicators = new ArrayList<>();
        for (String s: indics) {
            indicators.add(new ReportIndicator(s, "not processed yet", description));
        }
    }
    ...
    public boolean inCategory(String indicator){
        for (ReportIndicator i: indicators()) {
            if(i.name().equalsIgnoreCase(indicator)){
                return true;
            }
        }
        return false;
